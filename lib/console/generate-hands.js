...
async function main() {
  ...
  // Parse JSON5-like format (JavaScript object notation with unquoted keys)
  // Note: all.json is a trusted configuration file in the repository
  // Using Function constructor as a safer alternative to direct eval
  const parseJSONLike = new Function('return ' + jsonContent);
  const jsonObject = parseJSONLike();
